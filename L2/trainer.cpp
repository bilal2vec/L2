...
template <class T>
Tensor<double> Trainer<T>::fit_batch(Tensor<T> x, Tensor<T> y)
{
    Tensor<double> y_hat = sequential->forward(x);

    Tensor<double> loss = criterion->forward(y_hat, y);
    Tensor<double> derivative = criterion->backward();

    Tensor<double> zz = sequential->backward(derivative);
...
template <class T>
void Trainer<T>::fit(Tensor<T> x, Tensor<T> y, int epochs, int batch_size)
{
    ...
    for (int epoch = 0; epoch < epochs; ++epoch)
    {
        ...
        for (int batch = 0; batch < n_batches; ++batch)
        {
            ...
            Tensor<T> x_batch = x({{start, stop}});
            Tensor<T> y_batch = y({{start, stop}});
...
template <class T>
Tensor<T> Trainer<T>::predict(Tensor<T> x)
{
    int length = x({{0, -1}, {0, 1}}).length();
